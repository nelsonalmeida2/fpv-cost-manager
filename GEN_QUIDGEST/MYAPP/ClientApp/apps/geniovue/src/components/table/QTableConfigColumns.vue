...
<script>
	...
	export default {
		...
		methods: {
				...
				if (hiddenFilterCols.length > 0)
				{
					const buttons = {
						confirm: {
							...
							action: () => this.updateColumns(hiddenFilterCols)
						},
						cancel: {
							label: this.texts.noLabel,
							action: () => { this.tableConf.rows = this.getColumnsCfgRows() }
						}
					}
					displayMessage(this.texts.hideColumnConfirm, 'question', null, buttons)
				}
				else
					this.updateColumns()
			},
			 ...
			 */
			async onTableListRowReorder(event)
			{
				...
				document.getElementById(`column-config_row-${focusIndex}`)?.focus()
			},
