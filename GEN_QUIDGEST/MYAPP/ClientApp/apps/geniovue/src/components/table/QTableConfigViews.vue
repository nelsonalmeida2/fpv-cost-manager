...
<script>
	...
	export default {
		...
		methods: {
			...
			{
				return model ? `view-name-${model.id.value}` : ''
			},
			...
			updateViews()
			{
				...
				this.views.newElements.forEach((e) => {
					...
					const basedOnName = basedOn.id.value === 1 ? '' : basedOn.name.value
					const view = {
						name: e.name.value,
						basedOn: basedOnName
					}
		...
		watch: {
			...
			{
				this.views.elements.forEach((e) => {
					const name = e.id.value === 1 ? '' : e.name.value
