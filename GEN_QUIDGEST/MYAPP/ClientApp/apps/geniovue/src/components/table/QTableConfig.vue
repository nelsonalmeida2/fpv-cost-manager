...
<script>
	...
	export default {
		...
		{
			return {
				controlId: `${this.tableCtrl.id || this.tableCtrl.config.name || `q-table-${this._.uid}`}-config-menu`,
				modalId: 'config',
				configItems: [
					{
						key: 'views',
				...
				configHandlers: {
					...
					showView: (eventData) => this.confirmAndSetSelectedViewById(eventData),
		...
		methods: {
			...
			{
				const view = find(this.configViews, ['value', name])
				return view?.key ?? 1
			},
			...
			getViewNameById(id)
			{
				...
				const view = find(this.configViews, ['key', id])
				return view?.value
			},
