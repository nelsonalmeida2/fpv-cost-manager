	...
	export default {
		...
		methods: {
			...
			async focusSelectedFilter(filterIdx)
			{
				...
				if (filterIdx === this.selectedFilterIdx)
				{
					const filterForm = document.getElementById(`filter_${filterIdx}`)
		...
		watch: {
			...
			editedActiveFilters: {
				handler(val)
				{
					this.$emit('update:activeFilters', val)
				},
				deep: true
			},
			...
			editedGroupFilters: {
				handler(val)
				{
					this.$emit('update:groupFilters', val)
				},
				deep: true
			}
		}
