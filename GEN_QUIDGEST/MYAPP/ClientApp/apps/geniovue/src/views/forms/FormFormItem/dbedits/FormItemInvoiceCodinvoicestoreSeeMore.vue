<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-form-item-invoice-codinvoicestore-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FORM_ITEM__INVOICE__CODINVOICESTORE_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FormItemInvoiceCodinvoicestoreSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FORM_ITEM__INVOICE__CODINVOICESTORE_SeeMore', false),

				interfaceMetadata: {
					id: 'FORM_ITEM__INVOICE__CODINVOICESTORE_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FORM_ITEM__INVOICE__CODINVOICESTORE_SeeMore',
					name: 'FORM_ITEM__INVOICE__CODINVOICESTORE_SeeMore',
					controller: 'ITEM',
					action: 'FORM_ITEM_InvoiceValCodinvoicestore'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-form-item-invoice-codinvoicestore',
				dismissAction: this.close,
				returnElement: 'FORM_ITEM__INVOICE__CODINVOICESTORE_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.INVOICES44448),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-form-item-invoice-codinvoicestore')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Form_item_InvoiceValCodinvoicestore',
							serverMode: true,
							...
							defaultColumnSorting: {
								columnName: 'ValCodinvoicestore',
								sortOrder: 'asc'
							}
						},
						globalEvents: ['changed-INVOICE', 'changed-STORE'],
						uuid: 'Form_item_Form_item_InvoiceValCodinvoicestore',
