<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-form-item-subcategory-name-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FORM_ITEM__SUBCATEGORY__NAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FormItemSubcategoryNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FORM_ITEM__SUBCATEGORY__NAME_SeeMore', false),

				interfaceMetadata: {
					id: 'FORM_ITEM__SUBCATEGORY__NAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FORM_ITEM__SUBCATEGORY__NAME_SeeMore',
					name: 'FORM_ITEM__SUBCATEGORY__NAME_SeeMore',
					controller: 'ITEM',
					action: 'FORM_ITEM_SubcategoryValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-form-item-subcategory-name',
				dismissAction: this.close,
				returnElement: 'FORM_ITEM__SUBCATEGORY__NAME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.SUB_CATEGORIES30842),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-form-item-subcategory-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Form_item_SubcategoryValName',
							serverMode: true,
						...
						uuid: 'Form_item_Form_item_SubcategoryValName',
