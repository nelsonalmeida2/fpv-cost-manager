<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-form-invoice-store-name-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FORM_INVOICE__STORE__NAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FormInvoiceStoreNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FORM_INVOICE__STORE__NAME_SeeMore', false),

				interfaceMetadata: {
					id: 'FORM_INVOICE__STORE__NAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FORM_INVOICE__STORE__NAME_SeeMore',
					name: 'FORM_INVOICE__STORE__NAME_SeeMore',
					controller: 'INVOICE',
					action: 'FORM_INVOICE_StoreValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-form-invoice-store-name',
				dismissAction: this.close,
				returnElement: 'FORM_INVOICE__STORE__NAME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.STORES21606),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-form-invoice-store-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Form_invoice_StoreValName',
							serverMode: true,
