<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-form-photo-album-item-name-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FORM_PHOTO_ALBUM__ITEM__NAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FormPhotoAlbumItemNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FORM_PHOTO_ALBUM__ITEM__NAME_SeeMore', false),

				interfaceMetadata: {
					id: 'FORM_PHOTO_ALBUM__ITEM__NAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FORM_PHOTO_ALBUM__ITEM__NAME_SeeMore',
					name: 'FORM_PHOTO_ALBUM__ITEM__NAME_SeeMore',
					controller: 'PHOTOALBUM',
					action: 'FORM_PHOTO_ALBUM_ItemValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-form-photo-album-item-name',
				dismissAction: this.close,
				returnElement: 'FORM_PHOTO_ALBUM__ITEM__NAME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.ITEMS55321),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-form-photo-album-item-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Form_photo_album_ItemValName',
							serverMode: true,
							pkColumn: 'ValCoditem',
							tableAlias: 'ITEM',
