...
export class BaseControl
{
	...
	constructor(options, vueContext)
	{
		...
		this.componentOnLoadProc = asyncProcM.getProcListMonitor(`${options.id || crypto.randomUUID()}`, true)
...
export class LookupControl extends BaseControl
{
	...
	initHandlers()
	{
		...
		this.debouncedSearch = _debounce(this.handleSearch, 400)
...
export class TableListControl extends TableListBaseControl
{
	...
	get relatedFilterValues()
	{
		...
		this.globalFilters.forEach((filter) => {
			...
			const suffix = isList ? '_LIST' : ''

			let type = 'STRING'
			if (typeof value === 'number' || isList && typeof value[0] === 'number')
				type = 'NUMERIC'
			else if (typeof value === 'boolean' || isList && typeof value[0] === 'boolean')
				type = 'BOOLEAN'
	...
	hideConfig(modalId, removeModal = false)
	{
		const closeConfigPopup = () => {
			...
			if (removeModal || this.configIsDirty)
				this.removeFieldModal(modalId ?? 'config')
	...
	async updateConfig(config, hideConfig = false)
	{
		...
		if (typeof config.views !== 'undefined')
		{
			...
			deletedCurrent = config.views.some((e) => e.name === this.config.userTableConfigName && e.deleted)
		...
		if (reload)
		{
			...
			const configName = deletedCurrent && _isEmpty(newCurrentView) ? '' : undefined
	...
	reload(configName)
	{
		...
		return this.debouncedFetchData(params, this.fnHydrateViewModel)
	}
