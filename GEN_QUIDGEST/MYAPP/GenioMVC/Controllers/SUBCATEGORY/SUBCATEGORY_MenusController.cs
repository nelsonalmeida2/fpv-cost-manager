...
// USE /[MANUAL FPV MENU_GET 221]/
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "FPV_Menu_221_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_FPV_MENU_221.Name);
		...
		//
		// POST: /Subcategory/FPV_Menu_221_UploadFile
		[HttpPost]
		public ActionResult FPV_Menu_221_UploadFile(string importType, string qqfile) {
			FPV_Menu_221_ViewModel model = new FPV_Menu_221_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAsubcategory> rows = new List<CSGenioAsubcategory>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAsubcategory>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAsubcategory importRow in rows)
				{
					try
					{
						lineNumber++;
