...
// USE /[MANUAL FPV CONTROLLER_NAVIGATION ITEM]/
...
// USE /[MANUAL FPV MANUAL_CONTROLLER ITEM]/
					...
					case "FORM_ITEM__BRAND__NAME":	// Field (DB)
						{
							...
							model.Load_Form_item__brand__name(qs);
							result = model.TableBrandName;
						}
						break;
					case "FORM_ITEM__CATEGORY__NAME":	// Field (DB)
						{
							var model = new Form_item_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Form_item__category__name(qs);
			...
			try
			{
				...
				switch (string.IsNullOrEmpty(Identifier) ? "" : Identifier)
				{
					case "FORM_ITEM__BRAND__NAME":	// Field (DB)
						...
						break;
					case "FORM_ITEM__CATEGORY__NAME":	// Field (DB)
						values = new Form_item_ViewModel(UserContext.Current).GetDependant_Form_itemTableCategoryName(Selected);
						break;
					case "FORM_ITEM__SUBCATEGORY__NAME":	// Field (DB)
		...
		/// <summary>
		/// Recalculate formulas of the "Form_item" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Form_item([FromBody]Form_item_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "item",
				(primaryKey) => Models.Item.Find(primaryKey, UserContext.Current, "FFORM_ITEM"),
