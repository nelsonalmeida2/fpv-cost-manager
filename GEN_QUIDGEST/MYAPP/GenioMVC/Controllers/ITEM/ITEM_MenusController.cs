...
// USE /[MANUAL FPV MENU_GET 31]/
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "FPV_Menu_31_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_FPV_MENU_31.Name);
		...
		//
		// POST: /Item/FPV_Menu_31_UploadFile
		[HttpPost]
		public ActionResult FPV_Menu_31_UploadFile(string importType, string qqfile) {
			FPV_Menu_31_ViewModel model = new FPV_Menu_31_ViewModel(UserContext.Current);
