...
namespace GenioMVC.Controllers
{
	public partial class PhotoalbumController : ControllerBase
	{
		...
		public PhotoalbumController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL FPV CONTROLLER_NAVIGATION PHOTOALBUM]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAphotoalbum>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL FPV MANUAL_CONTROLLER PHOTOALBUM]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Photoalbum row = new Models.Photoalbum(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("photoalbum");
			...
			try
			{
				...
				{
					case "FORM_PHOTO_ALBUM__ITEM__NAME":	// Field (DB)
						{
							var model = new Form_photo_album_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Form_photo_album__item__name(qs);
							result = model.TableItemName;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "FORM_PHOTO_ALBUM__ITEM__NAME":	// Field (DB)
						values = new Form_photo_album_ViewModel(UserContext.Current).GetDependant_Form_photo_albumTableItemName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Form_photo_album" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Form_photo_album([FromBody]Form_photo_album_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "photoalbum",
				(primaryKey) => Models.Photoalbum.Find(primaryKey, UserContext.Current, "FFORM_PHOTO_ALBUM"),
				(model) => formData.MapToModel(model as Models.Photoalbum)
			);
		}
		...
		{
			return base.GetDocumsTickets("PHOTOALBUM", requestModel.FieldName, requestModel.KeyValue);
		}
