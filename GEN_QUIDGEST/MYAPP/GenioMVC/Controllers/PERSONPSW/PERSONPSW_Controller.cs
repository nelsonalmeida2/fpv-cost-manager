...
namespace GenioMVC.Controllers
{
	public partial class PersonpswController : ControllerBase
	{
		...
		public PersonpswController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL FPV CONTROLLER_NAVIGATION PERSONPSW]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioApersonpsw>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL FPV MANUAL_CONTROLLER PERSONPSW]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Personpsw row = new Models.Personpsw(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("personpsw");
			...
			try
			{
				...
				{
					case "FORM_PERSONPSW__PSW__NOME":	// Field (DB)
						{
							var model = new Form_personpsw_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Form_personpsw__psw__nome(qs);
							result = model.TablePswNome;
						}
						break;
					case "FORM_PERSONPSW__PERSON__NAME":	// Field (DB)
						{
							var model = new Form_personpsw_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Form_personpsw__person__name(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "FORM_PERSONPSW__PSW__NOME":	// Field (DB)
						values = new Form_personpsw_ViewModel(UserContext.Current).GetDependant_Form_personpswTablePswNome(Selected);
						break;
					case "FORM_PERSONPSW__PERSON__NAME":	// Field (DB)
						values = new Form_personpsw_ViewModel(UserContext.Current).GetDependant_Form_personpswTablePersonName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Form_personpsw" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Form_personpsw([FromBody]Form_personpsw_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "personpsw",
				(primaryKey) => Models.Personpsw.Find(primaryKey, UserContext.Current, "FFORM_PERSONPSW"),
				(model) => formData.MapToModel(model as Models.Personpsw)
			);
		}
		...
		{
			return base.GetDocumsTickets("PERSONPSW", requestModel.FieldName, requestModel.KeyValue);
		}
