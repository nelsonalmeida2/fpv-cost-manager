...
namespace GenioMVC.Controllers
{
	public partial class InvoiceController : ControllerBase
	{
		...
		public InvoiceController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL FPV CONTROLLER_NAVIGATION INVOICE]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAinvoice>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL FPV MANUAL_CONTROLLER INVOICE]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Invoice row = new Models.Invoice(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("invoice");
					...
					case "FORM_INVOICE__STORE__NAME":	// Field (DB)
						{
							var model = new Form_invoice_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Form_invoice__store__name(qs);
							result = model.TableStoreName;
						}
						break;
					default:
						break;
		...
		/// <summary>
		/// Recalculate formulas of the "Form_invoice" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Form_invoice([FromBody]Form_invoice_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "invoice",
				(primaryKey) => Models.Invoice.Find(primaryKey, UserContext.Current, "FFORM_INVOICE"),
		...
		{
			return base.GetDocumsTickets("INVOICE", requestModel.FieldName, requestModel.KeyValue);
		}
