...
{
	public class FPV_Menu_31_ViewModel : MenuListViewModel<Models.Item>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<FPV_Menu_31_RowViewModel> Menu { get; set; }
...
// USE /[MANUAL FPV LIST_LIMITS 31]/
		...
		public FPV_Menu_31_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="FPV_Menu_31_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public FPV_Menu_31_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public FPV_Menu_31_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<FPV_Menu_31_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Item.AddEPH<CSGenioAitem>(ref u, crs, "ML31");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Item.AddEPH<CSGenioAitem>(ref u, null, "ML31"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAitem> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet fpv_menu_31Conds = CriteriaSet.And();
			...
			conditions.SubSets.Add(fpv_menu_31Conds);
			fpv_menu_31Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL FPV OVERRQ 31]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAitem>(m_userContext, false, ref fpv_menu_31Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML31", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL FPV OVERRQLSTEXP 31]/
...
// USE /[MANUAL FPV OVERRQLIST 31]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAitem.GetInformation(), QMVC_POS_RECORD, sorts, fpv_menu_31Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAitem> listing = Models.ModelBase.Where<CSGenioAitem>(m_userContext, distinct, fpv_menu_31Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML31", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapFPV_Menu_31(listing);
		...
		private List<FPV_Menu_31_RowViewModel> MapFPV_Menu_31(ListingMVC<CSGenioAitem> Qlisting)
		{
			List<FPV_Menu_31_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFPV_Menu_31(row));
					i++;
				}
			}
		...
		/// to a FPV_Menu_31_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private FPV_Menu_31_RowViewModel MapFPV_Menu_31(CSGenioAitem row)
		{
			var model = new FPV_Menu_31_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL FPV VIEWMODEL_CUSTOM FPV_MENU_31]/
