...
{
	public class FPV_Menu_3311_ViewModel : MenuListViewModel<Models.Store>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<FPV_Menu_3311_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "19b34dc1-b550-4f22-8ed8-41e42cf5cb95";
		...
		public override CriteriaSet BaseConditions
		{
			get
			{
				...
				conds.Equal(CSGenioAstore.FldCodperson, Navigation.GetValue("person"));
		...
		{
// USE /[MANUAL FPV LIST_LIMITS 3311]/
		...
		public override int GetCount(User user)
		{
			...
			if (!qs.Joins.Select(x => x.Table).Select(y => y.TableAlias).Contains(CSGenio.business.Area.AreaPERSON.Alias))
				qs.Join(CSGenio.business.Area.AreaPERSON, TableJoinType.Inner).On(CriteriaSet.And().Equal(CSGenioAperson.FldCodperson, CSGenioAstore.FldCodperson));
		...
		public FPV_Menu_3311_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="FPV_Menu_3311_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public FPV_Menu_3311_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public FPV_Menu_3311_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<FPV_Menu_3311_RowViewModel>();
			...
			// Limitations
			// Limit "DB"
			crs.Equal(CSGenioAstore.FldCodperson, Navigation.GetValue("person"));
			if (isToExport)
			{
				// EPH
				crs = Models.Store.AddEPH<CSGenioAstore>(ref u, crs, "ML3311");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Store.AddEPH<CSGenioAstore>(ref u, null, "ML3311"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAstore> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet fpv_menu_3311Conds = CriteriaSet.And();
			...
			conditions.SubSets.Add(fpv_menu_3311Conds);
			fpv_menu_3311Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL FPV OVERRQ 3311]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAstore>(m_userContext, false, ref fpv_menu_3311Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML3311", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL FPV OVERRQLSTEXP 3311]/
			...
			if (tableReload)
			{
// USE /[MANUAL FPV OVERRQLIST 3311]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAstore.GetInformation(), QMVC_POS_RECORD, sorts, fpv_menu_3311Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAstore> listing = Models.ModelBase.Where<CSGenioAstore>(m_userContext, distinct, fpv_menu_3311Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML3311", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapFPV_Menu_3311(listing);
		...
		private List<FPV_Menu_3311_RowViewModel> MapFPV_Menu_3311(ListingMVC<CSGenioAstore> Qlisting)
		{
			List<FPV_Menu_3311_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFPV_Menu_3311(row));
					i++;
				}
			}
		...
		/// to a FPV_Menu_3311_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private FPV_Menu_3311_RowViewModel MapFPV_Menu_3311(CSGenioAstore row)
		{
			var model = new FPV_Menu_3311_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL FPV VIEWMODEL_CUSTOM FPV_MENU_3311]/
