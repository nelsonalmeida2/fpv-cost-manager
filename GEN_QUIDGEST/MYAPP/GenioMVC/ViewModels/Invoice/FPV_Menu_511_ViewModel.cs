...
{
	public class FPV_Menu_511_ViewModel : MenuListViewModel<Models.Invoice>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<FPV_Menu_511_RowViewModel> Menu { get; set; }
		...
		public override CriteriaSet BaseConditions
		{
			get
			{
				...
				conds.Equal(CSGenioAinvoice.FldCodperson, Navigation.GetValue("person"));
		...
		{
// USE /[MANUAL FPV LIST_LIMITS 511]/
		...
		public override int GetCount(User user)
		{
			...
			if (!qs.Joins.Select(x => x.Table).Select(y => y.TableAlias).Contains(CSGenio.business.Area.AreaPERSON.Alias))
				qs.Join(CSGenio.business.Area.AreaPERSON, TableJoinType.Inner).On(CriteriaSet.And().Equal(CSGenioAperson.FldCodperson, CSGenioAinvoice.FldCodperson));
		...
		public FPV_Menu_511_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="FPV_Menu_511_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public FPV_Menu_511_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public FPV_Menu_511_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<FPV_Menu_511_RowViewModel>();
			...
			if (!tableConfig.GroupFilters.ContainsKey("filter_FPV_Menu_511_PRICE"))
			{
				string defaultValue = "1";
				tableConfig.Filters.Add(new GroupFilter { Key = "filter_FPV_Menu_511_PRICE", Value = defaultValue });
			}

			{
				var groupFilters = CriteriaSet.Or();
				bool filter_FPV_Menu_511_PRICE_1 = false;
				if (tableConfig.GroupFilters.ContainsKey("filter_FPV_Menu_511_PRICE"))
					filter_FPV_Menu_511_PRICE_1 = tableConfig.GroupFilters["filter_FPV_Menu_511_PRICE"].Contains("1");
				else if (!tableConfig.GroupFilters.ContainsKey("filter_FPV_Menu_511_PRICE"))
					filter_FPV_Menu_511_PRICE_1 = true;
				if (filter_FPV_Menu_511_PRICE_1)
				{

				}

				bool filter_FPV_Menu_511_PRICE_2 = false;
				if (tableConfig.GroupFilters.ContainsKey("filter_FPV_Menu_511_PRICE"))
					filter_FPV_Menu_511_PRICE_2 = tableConfig.GroupFilters["filter_FPV_Menu_511_PRICE"].Contains("2");
				if (filter_FPV_Menu_511_PRICE_2)
				{
					groupFilters.Greater(CSGenioAinvoice.FldTotalprice, 100);

				}

				bool filter_FPV_Menu_511_PRICE_3 = false;
				if (tableConfig.GroupFilters.ContainsKey("filter_FPV_Menu_511_PRICE"))
					filter_FPV_Menu_511_PRICE_3 = tableConfig.GroupFilters["filter_FPV_Menu_511_PRICE"].Contains("3");
				if (filter_FPV_Menu_511_PRICE_3)
				{
				...
				bool filter_FPV_Menu_511_PRICE_4 = false;
				if (tableConfig.GroupFilters.ContainsKey("filter_FPV_Menu_511_PRICE"))
					filter_FPV_Menu_511_PRICE_4 = tableConfig.GroupFilters["filter_FPV_Menu_511_PRICE"].Contains("4");
				if (filter_FPV_Menu_511_PRICE_4)
				{
			...
			// Limitations
			// Limit "DB"
			crs.Equal(CSGenioAinvoice.FldCodperson, Navigation.GetValue("person"));
			if (isToExport)
			{
				// EPH
				crs = Models.Invoice.AddEPH<CSGenioAinvoice>(ref u, crs, "ML511");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Invoice.AddEPH<CSGenioAinvoice>(ref u, null, "ML511"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAinvoice> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet fpv_menu_511Conds = CriteriaSet.And();
			...
			conditions.SubSets.Add(fpv_menu_511Conds);
			fpv_menu_511Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL FPV OVERRQ 511]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAinvoice>(m_userContext, false, ref fpv_menu_511Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML511", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL FPV OVERRQLSTEXP 511]/
...
// USE /[MANUAL FPV OVERRQLIST 511]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAinvoice.GetInformation(), QMVC_POS_RECORD, sorts, fpv_menu_511Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAinvoice> listing = Models.ModelBase.Where<CSGenioAinvoice>(m_userContext, distinct, fpv_menu_511Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML511", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapFPV_Menu_511(listing);
		...
		private List<FPV_Menu_511_RowViewModel> MapFPV_Menu_511(ListingMVC<CSGenioAinvoice> Qlisting)
		{
			List<FPV_Menu_511_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFPV_Menu_511(row));
					i++;
				}
			}
		...
		/// to a FPV_Menu_511_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private FPV_Menu_511_RowViewModel MapFPV_Menu_511(CSGenioAinvoice row)
		{
			var model = new FPV_Menu_511_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL FPV VIEWMODEL_CUSTOM FPV_MENU_511]/
