<template>
	...
	<qtable 
		...
		<template #FPV="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'FPV' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					FPV : { active: true, Cod: "FPV", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.FPV.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.FPV.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.FPV.Description),
					name: "FPV",
					slot_name: "FPV",
					sort: false,
					visibility: vm.Modules.FPV.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
