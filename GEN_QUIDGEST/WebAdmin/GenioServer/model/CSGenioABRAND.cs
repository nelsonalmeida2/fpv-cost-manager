...
namespace CSGenio.business
{
	...
	/// Brand
	/// </summary>
	public class CSGenioAbrand : DbArea
	{
		/// <summary>
		...
		public CSGenioAbrand(User user, string module)
		{
            this.user = user;
            this.module = module;
			// USE /[MANUAL FPV CONSTRUTOR BRAND]/
		}

		public CSGenioAbrand(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codbrand", FieldType.KEY_INT);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "country", FieldType.KEY_INT);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "totalspending", FieldType.CURRENCY);
			Qfield.FieldDescription = "Total Spending";
			Qfield.FieldSize =  9;
			Qfield.MQueue = false;
		...
		private static void InicializaRelacoes(AreaInfo info)
		{
			...
			info.ChildTable[0]= new ChildRelation("item", new String[] {"brand"}, DeleteProc.NA);

			// Mother Relations
			//------------------------------
			info.ParentTables = new Dictionary<string, Relation>();
			info.ParentTables.Add("country", new Relation("FPV", "fpvbrand", "brand", "codbrand", "country", "FPV", "fpvcountry", "country", "codcountry", "codcountry"));
			info.ParentTables.Add("person", new Relation("FPV", "fpvbrand", "brand", "codbrand", "codperson", "FPV", "fpvperson", "person", "codperson", "codperson"));
		}
		...
		private static AreaInfo InicializaAreaInfo()
		{
			...
			info.QSystem="FPV";
			info.TableName="fpvbrand";
			info.ShadowTabName="";
			info.ShadowTabKeyName="";

			info.PrimaryKeyName="codbrand";
			info.HumanKeyName="name,".TrimEnd(',');
			info.Alias="brand";
			info.IsDomain = true;
			info.PersistenceType = PersistenceType.Database;
			info.AreaDesignation="Brand";
			info.AreaPluralDesignation="Brands";
			info.DescriptionCav="BRAND05002";
		...
		public static FieldRef FldCodbrand { get { return m_fldCodbrand; } }
		private static FieldRef m_fldCodbrand = new FieldRef("brand", "codbrand");

		/// <summary>Field : "" Tipo: "+" Formula:  ""</summary>
		public string ValCodbrand
		{
			get { return (string)returnValueField(FldCodbrand); }
			set { insertNameValueField(FldCodbrand, value); }
		}

		/// <summary>Field : "Name" Tipo: "C" Formula:  ""</summary>
		public static FieldRef FldName { get { return m_fldName; } }
		private static FieldRef m_fldName = new FieldRef("brand", "name");
		...
		private static FieldRef m_fldCountry = new FieldRef("brand", "country");
		...
		private static FieldRef m_fldDescription = new FieldRef("brand", "description");
		...
		private static FieldRef m_fldUpdated_at = new FieldRef("brand", "updated_at");
		...
		private static FieldRef m_fldUpdated_by = new FieldRef("brand", "updated_by");
		...
		private static FieldRef m_fldCreated_by = new FieldRef("brand", "created_by");
		...
		private static FieldRef m_fldCreated_at = new FieldRef("brand", "created_at");
		...
		private static FieldRef m_fldCodperson = new FieldRef("brand", "codperson");
		...
		/// <summary>Field : "Total Spending" Tipo: "$" Formula: SR "[ITEM->TOTALPRICE]"</summary>
		public static FieldRef FldTotalspending { get { return m_fldTotalspending; } }
		private static FieldRef m_fldTotalspending = new FieldRef("brand", "totalspending");

		/// <summary>Field : "Total Spending" Tipo: "$" Formula: SR "[ITEM->TOTALPRICE]"</summary>
		public decimal ValTotalspending
		{
			get { return (decimal)returnValueField(FldTotalspending); }
			set { insertNameValueField(FldTotalspending, value); }
		}
        ...
        public static CSGenioAbrand search(PersistentSupport sp, string key, User user, string[] fields = null, bool forUpdate = false)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAbrand area = new CSGenioAbrand(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAbrand> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAbrand>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAbrand> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAbrand>(where, listing);
        }
